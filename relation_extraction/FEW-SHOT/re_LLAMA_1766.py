...
def prepare_relation_tasks(ls_dataset):
    ...
    for item in ls_dataset:
        ...
        for r in annotations:
            ...
            if r_type == "labels":
                # SAFETY FIX FOR ENTITIES
                val = r.get("value", {})
                ls_labels = val.get("labels", [])
                
                if ls_labels and "text" in val:
                    entities.append({
                        "id": r["id"],
                        "text": val["text"],
                        "label": ls_labels[0]
                    ...
                    print(f"Skipping corrupt entity in Note ID {task_id}")
            
            elif r_type == "relation":
                from_id = r.get("from_id")
                to_id = r.get("to_id")
                # SAFETY FIX FOR RELATIONS
                labels = r.get("labels", [])
...
# save output
output_file = "re_results_llama3_1766.json"
...
with open("/ijc/LABS/SOLE/DATA/tfm_CLG/relation_extraction/ZERO-SHOT/report_LLAMA3_1766.txt", "w") as f:
