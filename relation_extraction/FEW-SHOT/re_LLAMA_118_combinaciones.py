...
dataset_path = Path("/ijc/LABS/SOLE/DATA/tfm_CLG/medical_ner/data/subsample_118_FIXED.json")
...
def prepare_relation_tasks(ls_dataset):
        ...
        for r in annotations:
            ...
            if r_type == "labels":
                try:
                    ...
                    print(f"Error en Entidad: Nota ID {task_id} tiene una etiqueta sin texto o categoría.")
                    continue
                ...
                if from_id and to_id and labels:
                    ...
                    motivo = "falta etiqueta" if not labels else "ID origen/destino perdido"
                    print(f" Relación Corrupta: Nota ID {task_id} ignorada porque {motivo}.")
                    if task_id not in notas_con_error:
                        notas_con_error.append(task_id)

        all_tasks.append({
            "id_original": task_id, # Guardamos el ID por si lo necesitas luego
            "text": text,
            "entities": entities,
            "relations": relations
        })
    ...
    if notas_con_error:
        print(f"\nTotal de notas con errores detectados: {len(notas_con_error)}")
        print(f"IDs a revisar en Label Studio: {notas_con_error}")
...
def get_candidate_pairs(entities):
    candidates = []
...
# define prompts
system_prompt = """Eres un experto médico. Tu tarea es evaluar una lista de PARES de entidades y decidir si existe una relación clínica entre ellas según las reglas establecidas y teniendo en cuenta el contexto que contiene la nota clínica completa.
...
for task in tqdm_cli(all_tasks):
    ...
    candidates = get_candidate_pairs(task["entities"])
    candidates_text = "\n".join([f"- {c['from']} -> {c['to']}" for c in candidates])
    user_prompt = f"""Analiza el siguiente texto médico y evalúa EXCLUSIVAMENTE si existen relaciones entre los pares de candidatos listados abajo.

Contexto: "{indexed_text}"

Candidatos a evaluar:
{candidates_text}

Instrucciones:
1. Para cada candidato, verifica si cumple alguna de las 8 reglas de tu sistema.
2. Si existe la relación, inclúyela en el JSON de salida.
3. Si no existe relación clínica, ignora ese par.
...
# save output
output_file = "re_results_llama3_118_comb.json"
...
with open("/ijc/LABS/SOLE/DATA/tfm_CLG/relation_extraction/ZERO-SHOT/report_LLAMA3_118_comb.txt", "w") as f:
    f.write("Relation Extraction Report\n")
    f.write("=" * 70 + "\n")
    f.write(report.to_string(index=False))

print("Report saved to report_LLAMA3_118_comb.txt")
