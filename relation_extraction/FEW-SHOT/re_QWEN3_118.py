...
- IMPORTANTE: Tu respuesta debe empezar directamente con '[' y terminar con ']'. No escribas 'Aquí tienes el JSON' ni uses bloques de código Markdown.
...
Salida:"""
    ...
    try:
        ...
        pred_relations = json.loads(json_payload)
    except Exception as e:
        print(f"Error parsing JSON. Raw Response: {raw_response[:50]}... | Error: {e}")
        pred_relations = []
...
# save output
output_file = "re_results_qwen3_118_v1.json"
...
with open("/ijc/LABS/SOLE/DATA/tfm_CLG/relation_extraction/ZERO-SHOT/report_QWEN3_118_v1.txt", "w") as f:
    f.write("Relation Extraction Report\n")
    f.write("=" * 70 + "\n")
    f.write(report.to_string(index=False))

print("Report saved to report_QWEN3_118_v1.txt")
