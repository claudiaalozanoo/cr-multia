    ...
    for item in ls_dataset:
        ...
        for r in annotations:
            if r["type"] == "labels":
                ...
                rel_labels = r.get("labels", ["NO_LABEL_FOUND"])
                relations.append({
                    "from_id": r["from_id"],
                    "to_id": r["to_id"],
                    "type": rel_labels[0]
...
# save output
output_file = "re_results_commandr_1766_v1.json"
...
with open("/ijc/LABS/SOLE/DATA/tfm_CLG/relation_extraction/ZERO-SHOT/report_COMMANDR_1766_v1.txt", "w") as f:
    f.write("Relation Extraction Report\n")
    f.write("=" * 70 + "\n")
    f.write(report.to_string(index=False))

print("Report saved to report_COMMANDR_1766_v1.txt")
