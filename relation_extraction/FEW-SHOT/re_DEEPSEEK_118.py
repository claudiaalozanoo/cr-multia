...
for task in tqdm_cli(all_tasks):
    ...
    user_prompt = f"""TEXTO: "{indexed_text}"
...
JSON:"""
    ...
    outputs = pipe(
        messages,
        max_new_tokens=256, # Clasificación no necesita 512, acórtalo para evitar alucinaciones
        do_sample=False,
        temperature=0,
        top_p=1,
    ...
    try:
        ...
        if clean_json:
            pred_relations = json.loads(clean_json)
        else:
            print(f"No JSON found: {raw_response[:50]}...")
            pred_relations = []
...
# save output
output_file = "re_results_deepseek_118_v1.json"
...
with open("/ijc/LABS/SOLE/DATA/tfm_CLG/relation_extraction/ZERO-SHOT/report_DEEPSEEK_118_v1.txt", "w") as f:
    f.write("Relation Extraction Report\n")
    f.write("=" * 70 + "\n")
    f.write(report.to_string(index=False))

print("Report saved to report_DEEPSEEK_118_v1.txt")
