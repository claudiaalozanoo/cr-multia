...
sft_config = SFTConfig(
    output_dir="/ijc/LABS/SOLE/DATA/tfm_CLG/relation_extraction/FINETUNNING/RESULTS_QWEN",
...
# save adapter
OUT_DIR_LLM = "/ijc/LABS/SOLE/DATA/tfm_CLG/relation_extraction/FINETUNNING/RESULTS_QWEN"
...
Salida:"""
        ...
        # 1. Parsing de la predicción
        try:
            pred_json = json.loads(re.search(r'\[.*\]', generated_text, re.DOTALL).group())
        except:
            pred_json = []

        # 2. Lógica de comparación para el reporte (Ground Truth vs Prediction)
        for rel_type in rel_labels:
            gt_set = set([
                (r.get('from_id', ''), r.get('to_id', '')) 
                for r in example["relations"] if r.get('type') == rel_type
            ])
            pred_set = set([
                (r.get('from_id', ''), r.get('to_id', '')) 
                for r in pred_json if isinstance(r, dict) and r.get('type') == rel_type
            ])

            metrics[rel_type]["tp"] += len(gt_set & pred_set)
            metrics[rel_type]["fp"] += len(pred_set - gt_set)
            metrics[rel_type]["fn"] += len(gt_set - pred_set)
    ...
    report_str = df_report.to_string(index=False)
    
    print("\nRelation Extraction Classification Report")
    print(report_str)
    
    return report_str, results_log
...
report, results = evaluate_re_model(model, tokenizer, test_data_prepared)

# save results
output_path = Path("/ijc/LABS/SOLE/DATA/tfm_CLG/relation_extraction/FINETUNNING/RESULTS_QWEN")
output_path.mkdir(parents=True, exist_ok=True)

with open(output_path / "qwen_results.json", "w", encoding="utf-8") as f:
    json.dump(results, f, indent=4, ensure_ascii=False)

with open(output_path / "report_QWEN_FINETUNNED_v1.txt", "w") as f:
    f.write(report)
