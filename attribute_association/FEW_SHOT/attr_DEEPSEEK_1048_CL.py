...
def generate_deepseek_prompt(task):
...
for task in tqdm_cli(valid_tasks):
    user_prompt, system_prompt = generate_deepseek_prompt(task)
    ...
    generated_part = outputs[0]["generated_text"].strip()

    raw_response = '{"attribute": ' + generated_part
...
with open("/ijc/LABS/SOLE/DATA/tfm_CLG/attribute_association/PLAIN/deepseek_results_1048.json", "w", encoding="utf-8") as f:
    json.dump(results, f, ensure_ascii=False, indent=4)
    
print("Results saved to deepseek_results_1048.json")
...
with open("/ijc/LABS/SOLE/DATA/tfm_CLG/attribute_association/PLAIN/report_DEEPSEEK_1048.txt", "w") as f:
    f.write("Attribute Classification Report\n")
    f.write(report)

print("Report saved to report_DEEPSEEK_1048.txt")
