...
def generate_commandr_prompt(task):
...
terminators = [
    ...
    tokenizer.convert_tokens_to_ids("<|END_OF_TURN_TOKEN|>")
]
...
for task in tqdm_cli(valid_tasks):
    user_prompt, system_prompt = generate_commandr_prompt(task)
...
with open("/ijc/LABS/SOLE/DATA/tfm_CLG/attribute_association/PLAIN/commandr_results_1048.json", "w", encoding="utf-8") as f:
    json.dump(results, f, ensure_ascii=False, indent=4)
    
print("Results saved to commandr_results_1048.json")
...
with open("/ijc/LABS/SOLE/DATA/tfm_CLG/attribute_association/PLAIN/report_COMMANDR_1048.txt", "w") as f:
    f.write("Attribute Classification Report\n")
    f.write(report)

print("Report saved to report_COMMANDR_1048.txt")
