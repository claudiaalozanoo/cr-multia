...
file_path = '/ijc/LABS/SOLE/DATA/tfm_CLG/medical_ner/data/subsample_236.json'
...
def get_evaluation_lists(all_notes_data, all_predictions):
    ...
    for i, entry in enumerate(all_notes_data):
        # Ground Truth de Label Studio
        gt_results = entry.get('annotations', [{}])[0].get('result', [])
        
        # Predicción (Accediendo como diccionario)
        # Accedemos a la clave 'prediction' y luego a la lista 'entities'
        pred_data = all_predictions[i]['prediction']
        pred_entities = pred_data.get('entities', [])
        
        # 1. Extraer etiquetas reales (Ground Truth)
        gt_map = {
        ...
        # Unimos todas las menciones únicas
...
with open("/ijc/LABS/SOLE/DATA/tfm_CLG/medical_ner/HF/mistral_results_236.json", "w", encoding="utf-8") as f:
...
with open("/ijc/LABS/SOLE/DATA/tfm_CLG/medical_ner/HF/medical_ner_report_MISTRAL_236.txt", "w") as f:
    f.write("Medical NER Classification Report\n")
    f.write(report)

print("Report saved to medical_ner_report_MISTRAL_236.txt")
