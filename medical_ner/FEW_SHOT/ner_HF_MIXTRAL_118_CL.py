...
print(f"--- SLURM JOB DIAGNOSTICS ---")
if torch.cuda.is_available():
    prop = torch.cuda.get_device_properties(0)
    print(f"GPU Device: {prop.name}")
    print(f"Total VRAM: {prop.total_memory / 1024**3:.2f} GB")
    # This checks how much memory is actually usable right now
    print(f"Currently Reserved: {torch.cuda.memory_reserved(0) / 1024**3:.2f} GB")
else:
...
with open("/ijc/LABS/SOLE/DATA/tfm_CLG/medical_ner/HF/mixtral_results_118.json", "w", encoding="utf-8") as f:
    json.dump(results, f, ensure_ascii=False, indent=4)
    
print("Results saved to mixtral_results_118.json")


# In[ ]:


with open("/ijc/LABS/SOLE/DATA/tfm_CLG/medical_ner/HF/medical_ner_report_MIXTRAL_118.txt", "w") as f:
    f.write("Medical NER Classification Report\n")
    f.write(report)

print("Report saved to medical_ner_report_MIXTRAL_118.txt")
